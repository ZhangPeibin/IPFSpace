...
export async function onRegenerateMnemonic(state, setState){
  const response = await fetch("/api/token/mnemonic");
  const json = await response.json();

  return setState({ ...state, mnemonic: json.mnemonic, loading: false });
}


export async function onRegenerateIdentityByMnemonic(mnemonic){
  const response = await fetch("/api/token/identity", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
export async function onAuth(state, setState,identity){
